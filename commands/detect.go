...
func init() {
	...
	// Data directory flag
	detectCmd.Flags().StringVar(&detectDataDir, "dir", "~/.claude/projects",
		"Data directory containing JSONL files")
...
func runDetect(cmd *cobra.Command, args []string) error {
	...
	config := &top.TopConfig{
		DataDir:             expandPath(detectDataDir),
	...
	if detectResetWindows {
		...
		// First pass: rebuild window history from data
		fmt.Println("Rebuilding window history from data...")
		_, err := orchestrator.LoadAndAnalyzeData()
		if err != nil {
			return fmt.Errorf("failed to rebuild window history: %w", err)
		}
		
		// Save the rebuilt window history
		if detector := orchestrator.GetDetector(); detector != nil {
			if windowHistory := detector.GetWindowHistory(); windowHistory != nil {
				if err := windowHistory.Save(); err != nil {
					util.LogWarn(fmt.Sprintf("Failed to save rebuilt window history: %v", err))
				} else {
					fmt.Println("Window history rebuilt and saved successfully.")
				}
			}
		}
	}
	...
	// Load and analyze sessions (second pass if reset, first pass if not)
	...
	// Limit sessions to display only the last 5
	sessionsToDisplay := sessions
	totalSessions := len(sessions)
	if totalSessions > 5 {
		sessionsToDisplay = sessions[:5] // Sessions are already sorted with most recent first
	}
	
	printSessions(sessionsToDisplay, aggregated, config.Timezone, config.TimeFormat, totalSessions)
	fmt.Println(util.FormatSectionSeparator())
...
func printSessions(sessions []*session.Session, aggregated *model.AggregatedMetrics, timezone, timeFormat string, totalSessions int) {
	fmt.Println(util.FormatDataTitle("=== Sessions ==="))
	
	// Show info about limited display if applicable
	if totalSessions > len(sessions) {
		fmt.Printf("Showing %d most recent sessions (out of %d total)\n\n", len(sessions), totalSessions)
	}
	...
	for i, sess := range sessions {
		...
		if len(sess.ModelDistribution) > 0 {
			...
			sort.Strings(models) // Sort models for consistent display order
