...
# Run e2e tests for TUI
.PHONY: test-e2e
test-e2e: build
	@echo "Running e2e tests..."
	@go test -v -tags=e2e ./commands -run TestTop

# Run all tests including e2e
.PHONY: test-all
test-all: test test-e2e

# Run quick TUI test (replaces test_tui_fix.sh)
.PHONY: test-tui-quick
test-tui-quick: build
	@echo "Running quick TUI test..."
	@go test -v -tags=e2e ./commands -run TestTopCommandStartup -short
...
help:
	...
	@echo "  test-e2e  - Run end-to-end TUI tests"
	@echo "  test-all  - Run all tests (unit + e2e)"
	@echo "  test-tui-quick - Run quick TUI startup test"
