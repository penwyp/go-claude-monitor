...
func TestNewTerminalDisplay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, model.ModeNormal, display.currentMode)
			assert.True(t, display.isFirstRender)
		})
	}
}
...
func TestTerminalDisplayScreenManagement(t *testing.T) {
	...
	assert.True(t, display.isFirstRender) // Should be set to true when entering alternate screen
...
func TestDisplayStateTransitions(t *testing.T) {
	...
	// Test help state transitions using display mode
	t.Run("help_transitions", func(t *testing.T) {
		...
		display.RenderWithState([]*Session{}, state)
		assert.Equal(t, model.ModeHelp, display.currentMode)
		
		// Hide help - back to normal
