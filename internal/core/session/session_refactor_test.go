...
func TestUnifiedDataSource(t *testing.T) {
	...
	// Get global timeline (uses aggregated data only for consistency)
	timeline := mc.GetGlobalTimeline(0)
	
	// Should have entries from aggregated data only (raw logs are for real-time updates)
	assert.Len(t, timeline, 1, "Should have entry from aggregated data")
	...
	// Should still have 2 entries due to deduplication (different hours)
	assert.Len(t, timeline, 2, "Should have entries from different hours")
}
...
func TestActiveSessionDetection(t *testing.T) {
	...
	t.Run("With existing sessions", func(t *testing.T) {
		...
		// With the new implementation, detectActiveSession doesn't create new sessions
		// It only verifies if current time is covered by existing sessions
		// Since the past session ended 1 hour ago and doesn't cover current time,
		// it should just return the existing sessions unchanged
		assert.Len(t, sessions, 1)
		assert.Equal(t, "past", sessions[0].ID)
	})
...
func TestIncrementalDetection(t *testing.T) {
	...
	// Use realistic timestamps that won't be rejected by window history validation
	now := time.Now().Unix()
	windowStart := now - 24*3600 // 1 day ago (fully historical)
	windowEnd := windowStart + 18000 // 5 hours later (session duration)
	...
	// Verify window is preserved (there might be multiple windows due to historical detection)
	windows := detector.windowHistory.GetLimitReachedWindows()
	assert.Greater(t, len(windows), 0, "Should have at least one limit window")
	
	// Find our specific window
	found := false
	for _, window := range windows {
		if window.StartTime == windowStart && window.Source == "limit_message" {
			found = true
			break
		}
	}
	assert.True(t, found, "Should find our specific limit window")
}
