...
func (m *WindowHistoryManager) ValidateNewWindow(proposedStart, proposedEnd int64) (validStart, validEnd int64, isValid bool) {
	...
	// First pass: Check for unexpired limit_message windows - they have absolute authority
	for _, record := range m.history.Windows {
		// Special handling for unexpired limit_message windows
		if record.IsLimitReached && record.Source == "limit_message" && record.EndTime > currentTime {
			// This is an unexpired limit window - it has absolute authority
			
			// Check for overlap
			if proposedStart < record.EndTime && proposedEnd > record.StartTime {
				// There's an overlap with an unexpired limit window
				// The limit window wins - reject or adjust the proposed window
				
				if proposedStart >= record.StartTime && proposedEnd <= record.EndTime {
					// Proposed window is entirely within the limit window - reject it
					util.LogInfo(fmt.Sprintf("Window validation: Rejecting window that overlaps with unexpired limit window %s-%s",
						time.Unix(record.StartTime, 0).Format("2006-01-02 15:04:05"),
						time.Unix(record.EndTime, 0).Format("2006-01-02 15:04:05")))
					// Return the limit window instead
					return record.StartTime, record.EndTime, false
				} else {
					// Partial overlap - adjust to avoid the limit window
					util.LogInfo(fmt.Sprintf("Window validation: Adjusting to avoid unexpired limit window %s-%s",
						time.Unix(record.StartTime, 0).Format("2006-01-02 15:04:05"),
						time.Unix(record.EndTime, 0).Format("2006-01-02 15:04:05")))
					
					if proposedStart < record.StartTime {
						// Proposed window starts before limit window - truncate at limit start
						validEnd = record.StartTime
						validStart = validEnd - constants.SessionDurationSeconds
						if validStart < 0 {
							// Can't fit a full window before the limit window
							validStart = record.EndTime
							validEnd = validStart + constants.SessionDurationSeconds
						}
					} else {
						// Proposed window overlaps end of limit window - start after it
						validStart = record.EndTime
						validEnd = validStart + constants.SessionDurationSeconds
					}
				}
	...
	// Second pass: Check against expired limit windows and other windows
	for _, record := range m.history.Windows {
		// Skip unexpired limit windows (already handled)
		if record.IsLimitReached && record.Source == "limit_message" && record.EndTime > currentTime {
			continue
		}
		
		// Handle expired limit_message windows
		if record.IsLimitReached && record.Source == "limit_message" {
			...
			proposedDate := time.Unix(validStart, 0).Format("2006-01-02")
			...
			if validStart < record.EndTime && validEnd > record.StartTime {
				...
				util.LogDebug(fmt.Sprintf("Window validation: Adjusting to avoid expired limit_message window %s-%s (same day overlap)",
					time.Unix(record.StartTime, 0).Format("2006-01-02 15:04:05"),
					time.Unix(record.EndTime, 0).Format("2006-01-02 15:04:05")))
				validStart = record.EndTime
				validEnd = validStart + constants.SessionDurationSeconds
...
func (m *WindowHistoryManager) UpdateFromLimitMessage(resetTime int64, messageTime int64, limitMessage string) {
	...
	// Check if this is an unexpired limit
	isUnexpired := resetTime > currentTime
	...
	// If this is an unexpired limit, remove any conflicting windows
	if isUnexpired {
		util.LogInfo(fmt.Sprintf("Processing UNEXPIRED limit message window: %s to %s",
			time.Unix(windowStart, 0).Format("2006-01-02 15:04:05"),
			time.Unix(windowEnd, 0).Format("2006-01-02 15:04:05")))
		
		// Remove conflicting windows
		m.RemoveConflictingWindows(windowStart, windowEnd)
	}
	...
	util.LogInfo(fmt.Sprintf("Updated window history from %s limit message: %s to %s (account-level, message: %s)",
		statusStr,
		time.Unix(windowStart, 0).Format("2006-01-02 15:04:05"),
		time.Unix(windowEnd, 0).Format("2006-01-02 15:04:05"),
		limitMessage))
...
// RemoveConflictingWindows removes windows that conflict with a given window range
func (m *WindowHistoryManager) RemoveConflictingWindows(windowStart, windowEnd int64) {
	m.history.mu.Lock()
	defer m.history.mu.Unlock()
	
	newWindows := make([]WindowRecord, 0)
	removedCount := 0
	
	for _, record := range m.history.Windows {
		// Check for overlap
		if record.StartTime < windowEnd && record.EndTime > windowStart {
			// This window conflicts - remove it
			removedCount++
			util.LogInfo(fmt.Sprintf("Removing conflicting window: %s-%s (source: %s) due to unexpired limit",
				time.Unix(record.StartTime, 0).Format("2006-01-02 15:04:05"),
				time.Unix(record.EndTime, 0).Format("2006-01-02 15:04:05"),
				record.Source))
		} else {
			// Keep non-conflicting windows
			newWindows = append(newWindows, record)
		}
	}
	
	m.history.Windows = newWindows
	
	if removedCount > 0 {
		util.LogInfo(fmt.Sprintf("Removed %d conflicting windows", removedCount))
	}
}
